...
PepXMLFile::PepXMLFile() :
		XMLHandler("", "1.12"), XMLFile("/SCHEMAS/pepXML_v114.xsd", "1.14"), proteins_(0), peptides_(
				0), experiment_(0), scan_map_(), rt_tol_(10.0), mz_tol_(10.0) {
...
void PepXMLFile::store(const String& filename, std::vector<ProteinIdentification>& protein_ids,
	...
	if (!protein_ids.empty()) {
		if (protein_ids.size() > 1) {
			warning(STORE,
					"More than one protein identification defined; only first one is written into pepXML, more are not supported.");
		}
	...
	f
			<< "<msms_pipeline_analysis date=\"2007-12-05T17:49:46\" xmlns=\"http://regis-web.systemsbiology.net/pepXML\" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://regis-web.systemsbiology.net/pepXML http://www.matrixscience.com/xmlns/schema/pepXML_v18/pepXML_v18.xsd\" summary_xml=\".xml\">"
			<< "\n";
	f << "<msms_run_summary base_name=\"" << File::basename(filename)
			<< "\" raw_data_type=\"raw\" raw_data=\".mzXML\" search_engine=\"" << search_engine_name
			<< "\">" << "\n";
		...
		if (it->getHits().size() > 0) {
			...
			if (h.getSequence().isModified()) {
				...
				if (p.hasNTerminalModification()) {
					n_term_mods.insert(
							ModificationsDB::getInstance()->getTerminalModification(
									p.getNTerminalModification(), ResidueModification::N_TERM).getFullId());
				}
				if (p.hasCTerminalModification()) {
					c_term_mods.insert(
							ModificationsDB::getInstance()->getTerminalModification(
									p.getCTerminalModification(), ResidueModification::C_TERM).getFullId());
				}

				for (Size i = 0; i != p.size(); ++i) {
					if (p[i].isModified()) {
						aa_mods.insert(
								ModificationsDB::getInstance()->getModification(
										p[i].getOneLetterCode(), p[i].getModification(),
										ResidueModification::ANYWHERE).getFullId());
					}
				}
			}
		}
	...
	for (set<String>::const_iterator it = n_term_mods.begin(); it != n_term_mods.end(); ++it) {
		...
		f << "      " << "<terminal_modification terminus=\"n\" massdiff=\""
				<< precisionWrapper(mod.getDiffMonoMass()) << "\" mass=\""
				<< precisionWrapper(mod.getMonoMass()) << "\" variable=\"Y\" description=\"" << *it
				<< "\" protein_terminus=\"\"/>" << "\n";
	}
	...
	for (vector<PeptideIdentification>::const_iterator it = peptide_ids.begin();
		...
		if (it->getHits().size() > 0) {
			if (it->getHits().size() > 1) {
				LOG_WARN << "PepXMLFile::store() : only writing the first peptide hit of "
						<< it->getHits().size() << " for PeptideID# " << count << "\n";
			}
			PeptideHit h = *it->getHits().begin();
			...
			f << "\t\t<spectrum_query spectrum=\"" << count << "\"" << " start_scan=\""
					<< scan_index << "\"" << " end_scan=\"" << scan_index << "\""
			...
			f << ">\n";
			f << "      <search_result>" << "\n";
			f << "\t\t\t<search_hit hit_rank=\"1\" peptide=\"" << seq.toUnmodifiedString()
					<< "\" peptide_prev_aa=\"" << h.getAABefore() << "\" peptide_next_aa=\""
					<< h.getAAAfter()
					<< "\" protein=\"Protein1\" num_tot_proteins=\"1\" num_matched_ions=\"0\" tot_num_ions=\"0\" calc_neutral_pep_mass=\""
					<< precisionWrapper(precursor_neutral_mass)
					<< "\" massdiff=\"0.0\" num_tol_term=\"0\" num_missed_cleavages=\"0\" is_rejected=\"0\" protein_descr=\"Protein No. 1\">"
					<< "\n";
			if (seq.isModified()) {
				f << "      <modification_info modified_peptide=\"" << seq << "\"";

				if (seq.hasNTerminalModification()) {
					const ResidueModification & mod =
							ModificationsDB::getInstance()->getTerminalModification(
									seq.getNTerminalModification(), ResidueModification::N_TERM);
					f << " mod_nterm_mass=\""
							<< precisionWrapper(
									mod.getMonoMass()
											+ seq[(Size) 0].getMonoWeight(Residue::Internal))
							<< "\"";
				}
				...
				if (seq.hasCTerminalModification()) {
					const ResidueModification & mod =
							ModificationsDB::getInstance()->getTerminalModification(
									seq.getCTerminalModification(), ResidueModification::C_TERM);
					f << "mod_cterm_mass=\""
							<< precisionWrapper(
									mod.getMonoMass()
											+ seq[seq.size() - 1].getMonoWeight(Residue::Internal))
							<< "\"";
				}
				...
				for (Size i = 0; i != seq.size(); ++i) {
					if (seq[i].isModified()) {
						const ResidueModification & mod =
								ModificationsDB::getInstance()->getModification(
										seq[i].getOneLetterCode(), seq[i].getModification(),
										ResidueModification::ANYWHERE);
						// the modification position is 1-based
						f << "         <mod_aminoacid_mass position=\"" << (i + 1) << "\" mass=\""
								<< precisionWrapper(
										mod.getMonoMass() + seq[i].getMonoWeight(Residue::Internal))
								<< "\"/>" << "\n";
					}
				}
...
void PepXMLFile::matchModification_(const DoubleReal mass, const String& origin,
		String& modification_description) {
	DoubleReal mod_mass = mass
			- ResidueDB::getInstance()->getResidue(origin)->getMonoWeight(Residue::Internal);
	vector<String> mods;
	ModificationsDB::getInstance()->getModificationsByDiffMonoMass(mods, origin, mod_mass, 0.015);
...
void PepXMLFile::makeScanMap_() {
	scan_map_.clear();
	Size scan = 0;
	for (MSExperiment<>::ConstIterator e_it = experiment_->begin(); e_it != experiment_->end();
			++e_it, ++scan) {
...
//			error(LOAD, "Cannot get precursor information - no experiment given");
		...
		if (spec.getMSLevel() == 2) {
			if (!use_precursor_data_) {
				...
				MSExperiment<>::ConstIterator it = experiment_->getPrecursorSpectrum(
						experiment_->begin() + scan);
				...
				if ((prec_mz > 0) && Math::approximatelyEqual(prec_mz, mz_, mz_tol_)
						&& (prec_rt > 0)
...
void PepXMLFile::load(const String& filename, vector<ProteinIdentification>& proteins,
		vector<PeptideIdentification>& peptides, const String& experiment_name) {
	MSExperiment<> exp;
...
void PepXMLFile::load(const String& filename, vector<ProteinIdentification>& proteins,
		vector<PeptideIdentification>& peptides, const String& experiment_name,
		const MSExperiment<>& experiment, bool use_precursor_data) {
...
void PepXMLFile::startElement(const XMLCh * const /*uri*/, const XMLCh * const /*local_name*/,
	...
	// now, elements occurring more frequently are generally closer to the top
		...
		// maybe  deal with meta data associated with PeptideProphet search
	...
	////NEW
		...
		if (optionalAttributeAsDouble_(mod_nterm_mass, attributes, "mod_nterm_mass")) { // this specifies a terminal modification
			...
			for (vector<AminoAcidModification>::const_iterator it = variable_modifications_.begin();
					it != variable_modifications_.end(); ++it) {
				if (mod_nterm_mass == it->mass && it->terminus == "n") {
					DoubleReal massdiff = (it->massdiff).toDouble();
					vector<String> mods;
					ModificationsDB::getInstance()->getTerminalModificationsByDiffMonoMass(mods,
							massdiff, 0.001, ResidueModification::N_TERM);
					if (!mods.empty()) {
						current_modifications_.push_back(make_pair(mods[0], 42)); // 42, because position does not matter
					} else {
						error(LOAD,
								String(
										"Cannot find N-terminal modification with mass "
												+ String(mod_nterm_mass) + "."));
					}
		...
		if (optionalAttributeAsDouble_(mod_cterm_mass, attributes, "mod_cterm_mass")) { // this specifies a terminal modification
			...
			for (vector<AminoAcidModification>::const_iterator it = variable_modifications_.begin();
				...
				if (mod_cterm_mass == it->mass && it->terminus == "c") {
					...
					vector<String> mods;
					ModificationsDB::getInstance()->getTerminalModificationsByDiffMonoMass(mods,
							massdiff, 0.001, ResidueModification::C_TERM);
					if (!mods.empty()) {
						current_modifications_.push_back(make_pair(mods[0], 42)); // 42, because position does not matter
					} else {
						error(LOAD,
								String(
										"Cannot find C-terminal modification with mass "
												+ String(mod_cterm_mass) + "."));
					}
					break; // only one modification should match, so we can stop the loop here
				}
			}
...
//			error(LOAD,
//					String("Cannot find modification '") + String(modification_mass) + " "
//							+ String(origin) + "' at position " + String(modification_position));
		}
		...
		if (is_variable == "Y") {
			if (aa_mod.description != "") {
				params_.variable_modifications.push_back(aa_mod.description); //
			} else {
			...
			if (aa_mod.description != "") {
				params_.fixed_modifications.push_back(aa_mod.description); //
			} else {
				String desc = aa_mod.aminoacid;
				if (aa_mod.massdiff.toDouble() >= 0) {
					desc += "+" + String(aa_mod.massdiff);
		...
		vector<String> mods;
		ModificationsDB::getInstance()->getModificationsByDiffMonoMass(mods, aa_mod.aminoacid,
				aa_mod.massdiff.toDouble(), 0.001);
		if (mods.size() > 1) {
			...
			error(LOAD,
					"Modification '" + String(aa_mod.mass)
							+ "' is not uniquely defined by the given data. Using '" + mods[0]
							+ "' to represent any of '" + mod_str + "'.");
		}
		...
		if (mass_type == "monoisotopic") {
			...
			if (mass_type != "average") {
				error(LOAD,
						"'precursor_mass_type' attribute of 'search_summary' tag should be 'monoisotopic' or 'average', not '"
								+ mass_type + "' (assuming 'average')");
			}
		}
		...
		if (mass_type == "monoisotopic") {
			...
			if (mass_type == "average") {
				...
				error(LOAD,
						"'fragment_mass_type' attribute of 'search_summary' tag should be 'monoisotopic' or 'average', not '"
								+ mass_type + "'");
			}
		}
		...
		if ((date[4] == ':') && (date[7] == ':') && (date[10] == ':')) {
			error(LOAD,
					"Format of attribute 'date' in tag 'msms_pipeline_analysis' does not comply with standard 'xs:dateTime'");
			date[4] = '-';
			date[7] = '-';
			date[10] = 'T';
		}
...
void PepXMLFile::endElement(const XMLCh * const /*uri*/, const XMLCh * const /*local_name*/,
	...
	if (wrong_experiment_) {
		...
		for (vector<pair<String, Size> >::const_iterator it = current_modifications_.begin();
				it != current_modifications_.end(); ++it) {
			// e.g. Carboxymethyl (C)
...
//				error(LOAD,
//						String("Cannot parse modification '") + it->first + "@" + it->second + "'");
			}
		}

		// fixed modifications
		for (vector<AminoAcidModification>::const_iterator it = fixed_modifications_.begin();
				it != fixed_modifications_.end(); ++it) {
			const Residue* residue = ResidueDB::getInstance()->getResidue(it->aminoacid);
			if (residue == 0) {
//				error(LOAD,
//						String("Cannot parse modification of unknown amino acid '") + it->aminoacid
//								+ "'");
			} else {
				DoubleReal new_mass = it->mass - residue->getMonoWeight(Residue::Internal);
				vector<String> mods;
				ModificationsDB::getInstance()->getModificationsByDiffMonoMass(mods, it->aminoacid,
						new_mass, 0.001);
				if (!mods.empty()) {
...
//					error(LOAD,
//							String("Cannot parse modification of amino acid '") + it->aminoacid
//									+ "'");
				}
			}
		}
		...
		current_peptide_.setMetaValue("scan_num", scan_num_);
